...
-- Vypiš všechny zastupce firmy Slunce
SELECT O.JMÉNO, O.PŘÍJMENÍ
FROM OSOBA O
JOIN FIRMA F ON O.ČÍSLO_LICENCE = F.ČÍSLO_LICENCE
WHERE F.OBCHODNÍ_JMÉNO = 'Slunce s.r.o';

-- Vypiš jméno trpaslíků prodávajících měď
SELECT JMÉNO,PŘÍJMENÍ
FROM OSOBA O JOIN KONTRAKT K on O.ID_OSOBY = K.ID_OSOBY
WHERE K.TYP_SUROVINY = 'Měď';

-- Vypiš všechny vlastníky dolu na měď
SELECT O.JMÉNO, O.PŘÍJMENÍ
FROM OSOBA O
JOIN VLASTNÍ V ON O.ID_OSOBY = V.ID_OSOBY
JOIN DUL D ON V.ID_DOLU = D.ID_DOLU
WHERE D.TYP_SUROVINY = 'Měď';

-- Kolik osob bydlí v Anor Londo
SELECT ADRESA_POBYTU, COUNT(*) AS POČET_OBYVATEL
FROM OSOBA
WHERE ADRESA_POBYTU = 'Anor Londo'
GROUP BY ADRESA_POBYTU;

-- V kolika dolech KAŽDÉ velikosti se těží železo
SELECT VELIKOST_DOLU, COUNT(*) AS POČET_DOLŮ
FROM DUL
WHERE TYP_SUROVINY = 'Železo'
GROUP BY VELIKOST_DOLU
ORDER BY
  CASE
    WHEN VELIKOST_DOLU = 'MALÝ' THEN 1
    WHEN VELIKOST_DOLU = 'STŘEDNÍ' THEN 2
    WHEN VELIKOST_DOLU = 'VELKÝ' THEN 3
  END;

-- Nalezne všechny doly, které mají přiřazený důl s kapacitou větší než 10 000
SELECT D.ID_DOLU, D.VELIKOST_DOLU, D.TYP_SUROVINY
FROM DUL D
WHERE EXISTS (
    SELECT 1
    FROM SKLAD S
    WHERE S.ID_SKLADU = D.ID_SKLADU
    AND S.KAPACITA > 10000
);

-- Nalezne všechny osoby, které spravují důl produkující Platinu nebo Palladium
SELECT O.ID_OSOBY, O.JMÉNO, O.PŘÍJMENÍ, O.RASA, D.TYP_SUROVINY
FROM OSOBA O
JOIN DUL D ON O.ID_OSOBY = D.ID_OSOBY
WHERE D.TYP_SUROVINY IN ('Platina', 'Palladium')
ORDER BY D.TYP_SUROVINY;